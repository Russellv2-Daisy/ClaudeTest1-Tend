...
Flow for connecting an account:
  1. GET  /aspsps?country=GB        -> find the Lloyds ASPSP entry
  2. POST /auth                     -> get a consent URL; user logs into Lloyds
...
class EnableBanking:
    def __init__(self, application_id: str, private_key_pem: str):
        ...
        # Render stores multiline secrets with literal \n sometimes — normalise.
    ...
    async def find_aspsp(self, name_contains="Lloyds", country="GB") -> dict:
        ...
        aspsps = data.get("aspsps", data if isinstance(data, list) else [])
        for a in aspsps:
